...
from .models import Colony, PetriDish # Assuming models.py will be updated for SubstanceTypeEnum
...
class CircleDetector:
            ...
            for x, y, r in circles[0,:]:
                ...
                if self._validate_dish_circle(processed, (x, y), r):
                    plates.append(PetriDish(
                        center=(int(x), int(y)),
                        radius=int(r),
                        # colonies=[],  # REMOVE THIS LINE - colonies is no longer a direct field of PetriDish
    ...
    def analyze_dish_contents(self, image: np.ndarray, dish: PetriDish) -> Tuple[DetectionMode, SubstanceType, List[Colony]]:
        ...
        if len(detected_substances) == 1:
            ...
            # 即使没有检测到明确物质，也可能属于单一抑菌圈在中心的情况
            logger.info("未检测到明确的抑菌物质，可能为单一抑菌圈在中心。")
            detection_mode = DetectionMode.SINGLE_SUBSTANCE # 默认为单一模式，后续会在中心搜索
            final_substance_type = SubstanceType.UNKNOWN # 类型未知
    ...
    def _detect_substances_by_type(self, image: np.ndarray, dish: PetriDish, substance_type: SubstanceType) -> List[Colony]:
        ...
        max_std_dev_val = 30.0 # Initial default max standard deviation
        ...
        if substance_type == SubstanceType.FILTER_PAPER:
            ...
            processed_for_hough = self.processor.preprocess(inverted_dish_roi)
            ...
            hough_param2_val = 12 # Lowered further for HOLEs accumulator threshold
            ...
            if self.px_per_mm and self.px_per_mm > 0:
                radius_factor_min = 0.80
                radius_factor_max = 1.20
        else:
            return []
    ...
    def process_image_pipeline(self, image: np.ndarray) -> Tuple[Optional[np.ndarray], List[Dict], Dict]:
        """
        完整的图像处理和检测流程。
        返回: (绘制结果的图像, 检测结果列表, 附加信息)
        """
        logger.info("开始完整的图像处理流程...")
        output_image = image.copy()
        all_dishes_results = [] # 未来可以支持多培养皿
        extra_info = {
            'petri_dishes_detected': 0,
            'substances_detected_total': 0,
            'inhibition_zones_detected_total': 0,
            'px_per_mm': None,
            'active_dish_details': [] # 存储每个处理的培养皿的详细结果
        }

        # 1. 检测培养皿
        dishes = self.detect_petri_dishes(image) # self.px_per_mm 会在内部被第一个有效培养皿设置
        if not dishes:
            logger.warning("未检测到培养皿。")
            return image, [], extra_info
        
        extra_info['petri_dishes_detected'] = len(dishes)
        extra_info['px_per_mm'] = self.px_per_mm # 使用最后一次标定的px_per_mm
        ...
        for dish_idx, current_dish_obj in enumerate(dishes):
            ...
            # 2. 分析当前培养皿内容
            # analyze_dish_contents 会更新 self.detection_mode, self.substance_type, self.detected_substances
            ...
            dish_result_summary = {
                ...
                'substances_count': len(self.detected_substances), # 使用 self.detected_substances
                'zones_results': []
            }
            ...
            if not self.detected_substances and mode == DetectionMode.SINGLE_SUBSTANCE:
                ...
                default_substance_radius_mm = self.hole_diameter_mm / 2 # 使用孔洞或滤纸片的一半作为虚拟点半径
                if self.px_per_mm and self.px_per_mm > 0:
                ...
                center_substance = Colony(
                    ...
                    # substance_type can be set to UNKNOWN here if Colony model supports it
                )
                self.detected_substances = [center_substance] # 更新当前培养皿的物质列表
                dish_result_summary['substances_count'] = 1
                extra_info['substances_detected_total'] += 1 # 更新总数
            ...
            for sub_idx, sub_obj in enumerate(self.detected_substances):
                color = (255, 100, 100) if self.substance_type == SubstanceType.FILTER_PAPER else (100, 100, 255) # 淡蓝纸，淡红孔
            ...
            # 3. 检测当前培养皿的抑菌圈 (detect_inhibition_zones 内部使用 self.detected_substances)
            ...
            all_dishes_results.extend(current_dish_zone_results_list) # 汇总所有抑菌圈结果
            ...
            for zone_res_dict in current_dish_zone_results_list:
                ...
                if primary:
                    cv2.circle(output_image, primary['center'], primary['radius'], (50, 200, 200), 2) # 蓝绿色抑菌圈
